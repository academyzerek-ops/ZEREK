...
# data/ лежит в корне репо, а main.py в api/ — поднимаемся на уровень выше
ROOT_DIR = os.path.dirname(BASE_DIR)
DATA_DIR = os.path.join(ROOT_DIR, "data")

# Если data/ рядом с main.py — используем его (для локального тестирования)
...
try:
    ...
    print(f"niches_dir exists: {os.path.exists(niches_dir)}")
    if os.path.exists(niches_dir):
        print(f"niches files: {os.listdir(niches_dir)}")
...
def root():
    ...
    return {"service":"ZEREK API","version":"3.0.0","status":"running","niches_loaded":n,"docs":"/docs"}
...
def get_formats_ep(niche_id:str):
...
def get_survey_ep(niche_id:str):
    if db is None: raise HTTPException(503,"БД не загружена")
...
def get_products_ep(niche_id:str,format_id:str,cls:str):
    if db is None: raise HTTPException(503,"БД не загружена")
...
def get_marketing_ep(niche_id:str,format_id:str,cls:str):
    if db is None: raise HTTPException(503,"БД не загружена")
...
def get_insights_ep(niche_id:str,format_id:str,cls:str):
    if db is None: raise HTTPException(503,"БД не загружена")
