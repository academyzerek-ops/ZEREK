...
def _build_vrd_priorities(result: dict) -> List[Dict[str, str]]:
    ...
    if coefs:
        try:
            ...
            # R7 G.2: «откладывайте деньги» — для соло-HOME совет вне
            # контекста (фиксов почти нет, поток меньше = просто
            # меньше доход в этом месяце). Меняем на полезную
            # механику: «не пугайтесь + усильте маркетинг в месяц
            # до просадки чтобы залить базу».
            fmt_id = (inp_p.get("format_id") or "").upper()
            is_solo = fmt_id.endswith("_HOME") or fmt_id.endswith("_SOLO")
            month_idx_prev = (min_idx - 1) % 12
            prev_month = months_short[month_idx_prev] if month_idx_prev < len(months_short) else ""
            prev_full_map = {
            ...
            prev_loc = prev_full_map.get(str(prev_month).lower(), str(prev_month).lower())
            if drop_pct >= 5:
                if is_solo:
                    out.append({
                        "title": f"Сезонная просадка {month_genitive}",
                        "detail": (
                            f"Поток клиентов падает примерно на {drop_pct}% "
                            "от обычного. Для вашего формата это значит "
                            "меньший доход в этом месяце — не уход в "
                            f"минус (фикс-расходов почти нет). Что "
                            f"работает: усилить маркетинг в {prev_loc}, "
                            "чтобы залить базу записями на просадочный "
                            "месяц заранее. Тогда спад пройдёт спокойно, "
                            "без паники и попыток демпинговать."
                        ),
                    })
                else:
                    out.append({
                        ...
                        "detail": (
                            f"Поток клиентов падает примерно на {drop_pct}% "
                            "от обычного. У вас остаются те же фикс-"
                            "расходы (аренда, ФОТ) — этот месяц нужно "
                            f"планировать заранее. Что работает: усилить "
                            f"маркетинг в {prev_loc} для записей на "
                            "просадочный месяц, либо запланировать "
                            "техническую паузу/обучение команды."
                        ),
                    })
            else:
                out.append({
                    ...
                    "detail": (
                        ...
                        "плюс — кассовые манёвры между месяцами не нужны."
                    ),
                })
...
def _direction_character(profit_monthly: int, avg_salary: int, payback_months: int) -> Tuple[str, str]:
    """R9 K.2: 6-градационная шкала «Характер направления».

    Возвращает (заголовок, объяснение) по отношению прибыли на мощности
    к средней ЗП региона. Шкала и тексты согласованы с ТЗ R9 — задают
    нейтральный, региональный язык вместо статичной фразы «невысокий
    доход» во всех городах (раньше Туркестан с ratio 91% получал тот же
    «невысокий», что Астана с 50% — это неверно).

    Шкала:
      ratio < 0.45         LOW         «Низкий доход для соло-формата»
      0.45 ≤ ratio < 0.65  MODEST      «Невысокий доход для соло-формата»
      0.65 ≤ ratio < 0.85  MIDDLE      «Средний доход — на уровне начинающего сотрудника»
      0.85 ≤ ratio < 1.05  DECENT      «Достойный доход — сопоставимый с наёмной работой»
      1.05 ≤ ratio < 1.30  HIGH        «Высокий доход — выше средней ЗП региона»
      ratio ≥ 1.30         VERY_HIGH   «Очень высокий доход — значительно выше региона»

    Сохранена защита от patho-кейсов (payback >24 мес, прибыль ≤0).
    """
    if avg_salary <= 0:
        avg_salary = 430_000
    ...
    if ratio < 0.45:
        return (
            ...
            "подходит как первый шаг в нишу — научиться обслуживать "
            "клиентов, обкатать процессы, накопить базу, — но как "
            "основной источник дохода будет проигрывать найму. "
    ...
    if ratio < 0.65:
        return (
            ...
            "шага в нишу без сильных рисков потери капитала. Хороший "
    ...
    if ratio < 0.85:
        return (
            ...
            "быть основным источником дохода, но без подушки и при росте "
    ...
    if ratio < 1.05:
        return (
            ...
            "основным заработком при условии что вы готовы к рискам ИП: "
    ...
    if ratio < 1.30:
        return (
            ...
            "становится действительно выгодной альтернативой найму. "
            "Главные ограничители — физический потолок и сезонные просадки. "
            "Дальнейший рост — только через SOLO с расширением "
            "параллельных мест.",
        )
    return (
        "Очень высокий доход — значительно выше региона",
        "Существенно выше средней ЗП по региону. Этот сценарий редко "
        "достижим в первом году соло-формата — вероятно либо завышен "
        "средний чек или загрузка в анкете, либо у вас уже есть сильная "
        "клиентская база. Перепроверьте входные параметры на стр. 2.",
    )
...
def _build_vrd_ctx(result: dict) -> dict:
    ...
    # R9 K.2: процент прибыли от ЗП — нужен для строки «около Z% от ЗП»
    # под подписью «Характер направления».
    income_ratio_pct = int(round((profit_for_dir / max(avg_salary, 1)) * 100)) if profit_for_dir > 0 else 0
    return {
        "level": color,
        ...
        "income_ratio_pct": income_ratio_pct,
    }
