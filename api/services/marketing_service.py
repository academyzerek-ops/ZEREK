...
def _compose_content_advice(niche_data: Dict[str, Any], archetype_id: str, self_produced: bool) -> str:
    ...
    if visual_score >= 4:
        return (
            "Ваша ниша — про визуал. Указанный маркетинговый бюджет "
            "рассчитан на самостоятельное ведение контента. Если у вас "
            "иначе — адаптируйте.\n\n"
            "<strong>Путь 1 — делаю всё сам</strong> (рекомендуется для "
            "соло-старта). Сами снимаете на телефон, монтируете в "
            "Capcut/Inshot, ведёте сторис, настраиваете таргет в "
            "кабинете Meta или через простой сервис. Весь бюджет идёт "
            "на рекламу — это и есть текущий расчёт. Минусы: нужно "
            "уметь, занимает 1-2 часа в день.\n\n"
            "<strong>Путь 2 — нанимаю помощников</strong> (для тех, у "
            "кого нет времени или навыка). Мобилограф (съёмка + базовый "
            "монтаж): 50-100К ₸/мес. Таргетолог: 30-60К ₸/мес или "
            "10-15% от рекламного бюджета. В этом сценарии 80-160К "
            "₸/мес уходят на людей — рекламный бюджет автоматически "
            "уменьшается на эту сумму, либо общий маркетинговый бюджет "
            "нужно увеличить на ту же сумму.\n\n"
            "Что хуже: уменьшать рекламу. Контент без денег на трафик "
            "не работает — это пустая работа. Если выбираете Путь 2 — "
            "закладывайте +80-160К ₸/мес сверх плана."
        )
