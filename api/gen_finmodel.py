...
def generate_finmodel(
    ...
    # Дата в футере (строка 55 — безопасно ищем)
    ...
    # Пишем только в НЕ-merged ячейки. Если merged — пропускаем.
    sheets_titles = {
        '📋 НАВИГАЦИЯ': {
            'A1': f'="ФИНАНСОВАЯ МОДЕЛЬ — "&UPPER({P}!C5)',
        },
        '⚙️ ПАРАМЕТРЫ': {
            'A1': f'="ПАРАМЕТРЫ МОДЕЛИ — "&UPPER(C5)',
        },
        '📊 ДОПУЩЕНИЯ': {
            'A1': f'="ДОПУЩЕНИЯ МОДЕЛИ — "&UPPER({P}!C5)',
        },
        '🎯 ДАШБОРД': {
            'A1': f'="ДАШБОРД — "&UPPER({P}!C5)&" ("&{P}!C6&")"',
        },
        '📈 P&L': {
            'A1': f'="P&L — "&UPPER({P}!C5)&" ("&{P}!C6&")"',
        },
        '💰 CASH FLOW': {
            'A1': f'="CASH FLOW — "&UPPER({P}!C5)&" ("&{P}!C6&")"',
        },
        '📑 БАЛАНС': {
            'A1': f'="БАЛАНС — "&UPPER({P}!C5)',
        },
    }
